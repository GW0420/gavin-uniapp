...
const config = {
	// 信任的标签（保持标签名不变）
	trustTags: makeMap(
		'a,abbr,ad,audio,b,blockquote-cls,br,code,col,colgroup,dd,del,dl,dt,div,em,fieldset,h1,h2,h3,h4,h5,h6,hr,i,img,ins,label,legend,li,ol,p,q,ruby,rt,source,span,strong,sub,sup,table,tbody,td,tfoot,th,thead,tr,title,ul,video'
		),
...
Parser.prototype.popNode = function() {
			...
			for (let row = 1; row <= trList.length; row++) {
				...
				for (let j = 0; j < trList[row - 1].children.length; j++) {
					...
					if (td.name === 'td' || td.name === 'th') {
						...
						style += (border ? `;border:${border}px solid gray` + (spacing ? '' :
	...
	if (children.length >= 50 && node.c && !(styleObj.display || '').includes('flex')) {
		...
		for (let j = i; j >= -1; j--) {
			// 合并多个块级标签
			if (j === -1 || children[j].c || !children[j].name || (children[j].name !== 'div' && children[j]
					.name !== 'p' && children[j].name[0] !== 'h') || (children[j].attrs.style || '').includes(
					'inline')) {
				if (i - j >= 5) {
